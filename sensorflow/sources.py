...
class subscribe(Source):
    """ Accepts messages from Foghorn zmq broker

    Parameters
    ----------
    topics: list of str
        Labels of topics to consume from
    connection_params: dict
        Settings to set up the stream, see
        Examples:
        message bus server, Connection string(s) (host:port) by which to reach
        message bus;
    poll_interval: number
    ...
    >>> source = Stream.from_zmq(['mytopic'],
    ...           {'MESSAGE_BUS_HOST': 'localhost',
    ...           'MESSAGE_BUS_PUB_PORT', 9501})  # doctest: +SKIP

    """
    def __init__(self, topics, connection_params, poll_interval=0.1, start=False, **kwargs):
        import zmq
        ...
        try:
            ...
            pass
        self._mbus_sub_port = int(connection_params['MESSAGE_BUS_SUB_PORT'])
        self._context = zmq.Context()
        try:
            self._subscriber = self._context.socket(zmq.SUB)
            mbus_sub_host = "tcp://" + self._mbus_host + ":"  + str(self._mbus_sub_port)
            self._subscriber.connect(mbus_sub_host)
        ...
        super(subscribe, self).__init__(ensure_io_loop=True, **kwargs)
        self.stopped = True
        if start:
            self.start()
    ...
    def do_poll(self):
        if self._subscriber is not None:
            if self.poller.poll(self.poll_interval):
                msgparts = self._subscriber.recv_multipart()
                return msgparts[1]
    ...
    def start(self):
        ...
        if self.stopped:
            self.stopped = False
            for topic in self.topics: self._subscriber.setsockopt(zmq.SUBSCRIBE, topic.encode('ascii', 'replace'))
            self.loop.add_callback(self.poll_zmq)

    def _close_consumer(self):
        import zmq
        if self._subscriber is not None:
            subscriber = self._subscriber
            for topic in self._topics: subscriber.setsockopt(zmq.UNSUBSCRIBE, topic.encode('ascii', 'replace'))
            subscriber = None
